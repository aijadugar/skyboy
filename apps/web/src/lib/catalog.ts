...
export interface Skill {
  ...
  // is a model folder that carries its own SKILL.md plus a nested skills/
  // sub-folder. `provider` is set on nested children (the provider slug they
  // live under); `isProvider` marks the container itself; the count drives the
  // browse card ("N skills").
...
function readSkill(skillDir: string, repoPath: string, provider: string | null = null): Skill | null {
  ...
  // A model-provider folder is a container (a "model" that holds skills), not a
  // leaf skill: skills/model-providers/<provider>/skills/<slug>. Children under
  // <provider>/skills/ come back with `provider` set.
...
// Content roots: the community-contributed skills/ tree that lives at the repo
// root. Overridable via SKYBOY_SKILLS_ROOT (for deploys that vendor the content
// elsewhere); the default walks up from the app dir to the repo root.
