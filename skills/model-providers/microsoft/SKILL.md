---
name: microsoft
description: Integration guide and best practices for using Microsoft as a model provider.
license: MIT
---

## Command

Install this skill with:

```bash
skyboy add microsoft
```

Or preview it first with `skyboy info microsoft`.

# Microsoft

Use this skill when integrating with Microsoft as a model provider. Covers API setup, authentication, model selection, and best practices for production use.

## When it applies

- Setting up Microsoft API credentials and SDK
- Choosing the right model from Microsoft's offerings
- Handling rate limits, retries, and error responses
- Optimizing cost and latency with Microsoft
- Streaming responses and tool use with Microsoft

---

## Quick start

```bash
# Install the SDK (example — adapt to the provider's actual package)
npm install @microsoft/sdk
```

```typescript
import { MicrosoftClient } from "@microsoft/sdk";

const client = new MicrosoftClient({
  apiKey: process.env.MICROSOFT_API_KEY,
});
```

---

## Model selection

Microsoft offers models across capability tiers. Choose based on your use case:

| Use case | Recommended model |
|---|---|
| General chat | Check Microsoft's latest flagship model |
| Code generation | Look for code-tuned variants |
| Fast inference | Use smaller/distilled models |
| Long context | Check context window limits |

---

## Best practices

- **Rate limits**: Respect Microsoft's published RPM/TPM limits; implement exponential backoff.
- **Streaming**: Use streaming for interactive UIs to reduce time-to-first-token.
- **Cost control**: Cache responses where possible; use shorter prompts with system instructions.
- **Error handling**: Map Microsoft's error codes to user-friendly messages; never leak API keys client-side.

## References

- Microsoft official documentation
- Microsoft API reference

## Microsoft Plugins

Azure's public plugin ecosystem (indexed under `plugins/` in this folder; no dedicated skills repositories published to date):

| Repository | Description | Language | License |
|---|---|---|---|
| [kubelogin](https://github.com/Azure/kubelogin) | Kubernetes credential (exec) plugin implementing Azure authentication | Go | MIT |
| [vagrant-azure](https://github.com/Azure/vagrant-azure) | Enable Vagrant to manage virtual machines in Microsoft (archived) | Ruby | MIT |
